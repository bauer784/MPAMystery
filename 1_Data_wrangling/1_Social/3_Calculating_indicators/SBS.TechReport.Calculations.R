...
Organization <- Organization %>%
  filter(MPAID < 17 & MPAID >14)
HHData$SettlementName<-as.character(HHData$SettlementName)
HHData$SettlementName<-gsub("Alila TImur","Alila Timur",HHData$SettlementName)
HHData$SettlementName<-as.factor(HHData$SettlementName)
...
MT %>%
  filter(RightsAccess==2) #REMOVE THIS ONE, INVALID RESPONSE? ????????????????????????????????? HH 6280
